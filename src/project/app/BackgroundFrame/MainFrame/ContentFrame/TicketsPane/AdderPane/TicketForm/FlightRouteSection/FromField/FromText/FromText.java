package project.app.BackgroundFrame.MainFrame.ContentFrame.TicketsPane.AdderPane.TicketForm.FlightRouteSection.FromField.FromText;
...
public class FromText extends Text
{
        ...
        (
            new ChangeListener<Number>()
            {
                public void changed(ObservableValue<? extends Number> observableValue, Number OldValue, Number NewValue)
                {
                    ...
                    if(FromLocation.get()!=0) {TextService.setAnimation(new Timeline(new KeyFrame(Duration.millis(50), new KeyValue(textProperty(), getLocationName(FromLocation.get())))));}
                    else {TextService.setAnimation(new Timeline(new KeyFrame(Duration.millis(50), new KeyValue(textProperty(), "From"))));}
                    TextService.restart();
                }
            }
        );
            ...
            new ChangeListener<Boolean>()
            {
                public void changed(ObservableValue<? extends Boolean> observableValue, Boolean OldValue, Boolean NewValue)
                {
                    if(NewValue==true)
                    {
                        ...
                        if(TextService.isRunning()==true) {TextService.cancel();}
                        TextService.setAnimation(new Timeline(new KeyFrame(Duration.millis(50), new KeyValue(textProperty(), "From"))));
